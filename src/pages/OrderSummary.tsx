...
const OrderSummary = () => {
  ...
  const generateTotal = () => {
    const totalBerat =
      (detailData.jumlah_karung ?? 0) * (detailData.berat_per_karung_kg ?? 0);
    return totalBerat * hargaPerKg[detailData.item];
  };
  ...
  const sendBilling = () => {
    ...
    const templateMessage = `
  Halo, saya ingin mengajukan penagihan untuk order berikut:\n
  Order ID: ${orderId}\n
  Jenis Paket: ${item}\n
  Alamat: ${alamat}\n
  Rekening Penagih: 3201070012\n
  BANK Penerbit: BCA\n
  Total Tagihan: ${formatRupiah(generateTotal())}
    `;
    ...
    <Layout>
      <div className="bg-gray-100 min-h-screen">
        ...
        <div className="p-6 py-24">
          ...
          {userDetails?.user_level === 1 && detailData?.billingStatus === 6 && (
            <div className="mt-4">
              <div className="rounded-md overflow-hidden">
                <div className="w-full grid grid-cols-2 items-center gap-4 bg-white py-2 px-4">
                  <Label className="text-gray-700 leading-none">
          ...
          {userDetails?.user_level === 2 &&
            (detailData?.status === 7 || detailData?.status === 3) && (
              <div className="mt-4">
